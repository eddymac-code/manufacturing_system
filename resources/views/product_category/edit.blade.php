...
@section('content')
    <div class="my-20 mx-auto bg-gray-300 rounded-lg w-4/12 py-1">
        <div class="flex align-center justify-between w-full p-4">
            <h2 class="text-blue-900 font-extrabold text-xl">Edit Category</h2>
            <a href="{{ route('dashboard') }}" 
            class="bg-gray-600 text-white font-bold px-4 py-2 rounded-lg">Go Back</a>
        </div>

        <form action="{{ route('edit-category', $category) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-4 px-4">
            ...
            <div class="mb-4 px-4 flex items-center space-x-6">
                @if ($category->image)
                    <div class="shrink-0">
                        <img class="h-16 w-16 object-cover rounded-full" src="{{ asset('storage/images/product_category/'.$category->image) }}"
